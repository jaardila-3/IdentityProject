...
public class RolesService(IUnitOfWork unitOfWork) : IRolesService
{
    ...
    public async Task<List<UserRolesDto>?> GetUserRolesByUserIdAsync(string userId)
    {
        var userRoles = await _unitOfWork.RolesRepository.GetUserRolesByUserIdAsync(userId) ?? [];
        return userRoles.Select(ur => new UserRolesDto(ur.UserId, ur.RoleId)).ToList() ?? [];
    }
